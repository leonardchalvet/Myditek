$(window).on('load', function() {
	...
	$('header.mobile .profil .notif .dropdown .more.close').click(function() {
		$(this).parent().removeClass('show');
	})

	/* DASHBOARD */
	$('.wrapper-tab-dashboard .container-el .el .container-ist .container-btn .btn').click(function(){
		$('.wrapper-tab-dashboard .container-el .el .container-ist .container-btn .btn.active').removeClass('active');
		$(this).addClass('active');
	})
	/* END DASHBOARD */
			...
			if($('main').hasClass('messagerie')) {
				...
				if(offsetTopMessageRead == undefined && heightMessageRead == undefined) {
				    ...
				    if ( scrollBottom >= (offsetTopMessageRead + heightMessageRead + 30)  ) {
				    	...
				    	let bool = $('body').hasClass('agent');
				    	let left = $('.wrapper-tab-messagerie .message-interaction .messages-action').offset().left + 100 + ( bool ? -30 : 0 );
					...
					$(this).children().children('.container-line').children().each(function() {
						...
						$(this).parent().parent().children('.time').children().each(function() {
							if(hs == $(this).attr('data-hour')) {
								...
								day.css( { 'top':( $(this).position().top + 10 + addt) + 'px',
									       'height':( (he - hs) * 43 + (he - hs) * 10 + addb + (he - hs - 1)) + 'px' });
							}	
						});
					});
				}
	...
	$('.wrapper-tab-profil .container-profil form.password .input input').each(function() {
		$(this).keyup(function() {
			if($(this).val().length > 0) {
				$('.wrapper-tab-profil .container-profil form.password .submit').addClass('validate');
			}
			else {
				$('.wrapper-tab-profil .container-profil form.password .submit').removeClass('validate');
			}
		})
	})

	$('.wrapper-tab-profil .container-profil form.information .input input').each(function() {
		$(this).keyup(function() {
			if($(this).val().length > 0) {
				$('.wrapper-tab-profil .container-profil form.information .submit').addClass('validate');
			}
			else {
				$('.wrapper-tab-profil .container-profil form.information .submit').removeClass('validate');
			}
		})
	})
	...
	*******************/
	...
	if (window.matchMedia("(max-width: 800px)").matches) {
		let h = $(window).height() - 68;
		console.log(h);
		$('header.mobile .nav').css("height",h+"px");
	}
})